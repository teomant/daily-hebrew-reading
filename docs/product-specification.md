...
Nature sites, parks, gardens, trails, viewpoints, landmarks, fortresses, and tourist destinations are not a required family. At most two of the 24 candidates may be place-led, and each needs an exceptional, specific human story rather than a generic preservation, visitor, or “nature and history meet” hook. At most one candidate may be led by archaeology. The first seven HISTORY candidates—and the deterministic Python selection of the seven published slots—contain at least two person stories, two Israeli-industry stories, two culture stories, and one event or exceptional place story; they contain no archaeology and at most one place. The first two discovery passes focus on Israeli material. Broader relatable world history is reserved for a third and final fallback when sourced slots remain open.
...
For a new complete issue, two EVERYDAY and two DIALOG stories are editorial targets, not validation requirements. The normal sourced target is four CURRENT and seven HISTORY stories. Discovery returns 12 CURRENT and 24 HISTORY screening candidates before filtering. Duplicate-reviewed but unselected HISTORY candidates remain in a reserve queue, with Israel-focused passes ordered ahead of the worldwide fallback. Deep research normally uses one batched call for the selected seven; insufficient or invalid packs are replaced from compatible reserves and researched in further batches until the slot succeeds or that reserve family is exhausted. A reserve is consumed at most once. Two consecutive HISTORY research request failures abort generation rather than silently publishing an all-generated substitute issue. Missing sourced slots do not expand generated planning beyond its normal four EVERYDAY/DIALOG stories, so a source shortfall may produce a smaller valid issue. The research prompt also receives compact date/type/category/ID/brief summaries from all issues in the previous three calendar days so CURRENT events, HISTORY subjects, EVERYDAY plots, and DIALOG situations are not repeated on consecutive days. Recent generated-scenario records are explicitly forbidden comparison data; an identical scenario ID or a renamed version with the same practical problem, interaction, and resolution must be discarded.
...
Read the recent scenario-history data, identify what has been used, remove overly similar ideas, normally select about two EVERYDAY situations, create scenario briefs, and adapt them into the issue's configured levels. Together, EVERYDAY and DIALOG planning is capped at the normal four generated stories for a fresh issue even when sourced slots are missing. Planning uses canonical broad domains and keeps generated domains distinct within the current issue, including a same-day append; prior-day domain reuse remains allowed because scenario history forbids repeated plots rather than broad subject areas. Vary interaction shapes as well as settings: no batch should consist only of delays, deliveries, missing items, repairs, access trouble, or workplace problems, and at least one normal batch scenario should be a positive cooperative activity. Prioritize reusable interaction with colleagues, managers, customers, neighbors, friends, strangers, and service workers. Mark every EVERYDAY story visibly as fully AI-generated in the selected interface language.

## 29. DIALOG generation

Aim for about two short dialogues for each new issue within the combined four-story generated allocation. Use ordinary situations useful to a learner and their family, normally with two speakers and 8–12 short alternating turns. Store every turn as its own paragraph-array item so it appears on a separate line. Every item starts with a consistent Hebrew speaker name and colon and contains only that speaker's direct words; speaker names remain stable across levels and adjacent turns alternate. A compact literal eight-turn example is supplied only to DIALOG adaptation calls, not to every article call. Keep the exchange natural, simple, and practical across all configured levels. Reuse the scenario history to avoid recently repeated situations, and mark every DIALOG visibly as fully AI-generated.

## 30. HISTORY generation

Return 24 distinct compact HISTORY screening candidates from a substantially larger search and normally select seven. Classify each as `person`, `israeliIndustry`, `culture`, `event`, `place`, or `archaeology`; this internal field is used for selection and removed before publication. The first two passes are Israel-focused and require model-reported discovery lanes with minimums of 12 Wikimedia/reference candidates, six National Library or Historical Jewish Press candidates, four state visual-archive candidates, and two Israeli culture-archive candidates. This lane field guides editorial variety; it is not verified URL provenance and is removed before adaptation and publication. A third pass may use only the `worldwideFallback` lane. Require at least six person, six Israeli-industry, six culture, and four event candidates. Permit at most two place candidates and one archaeology candidate. The selected seven must contain at least two person, two Israeli-industry stories, two culture stories, and one event or exceptional place story, with at most one place and no archaeology. Favor concrete subjects that appear likely to support interesting stories, easy explanations, and useful ordinary language. They need not relate to the current date or current events. Do not spend the screening response on story beats. Retain duplicate-reviewed unselected HISTORY candidates as reserves, then deeply research only the selected subjects in one batched web call. A sufficient pack has 8–12 ordered beats and required setup, action, turning-point, and outcome beats. Source links are optional and their absence alone never makes the pack insufficient. Invalid packs consume compatible reserves in queue order and repeat research until that family is exhausted.
...
Use the OpenAI API for sourced discovery and selection, sourced semantic duplicate review, selected-HISTORY research, generated-scenario planning, level adaptation, lexical segmentation, and Russian and English translations. Use the current official SDK and a modern API. A normal issue first makes up to three web-enabled CURRENT/HISTORY discovery requests that each return exactly 36 compact screening candidates rather than final stories. Every returned candidate batch then receives a separate no-web semantic duplicate review using only compact briefs and URLs before selection. Batched web-enabled HISTORY research starts with selected subjects and repeats only for unresolved compatible reserve replacements. A separate no-web EVERYDAY/DIALOG planning request creates at most the normal four generated stories for a fresh issue, followed by one structured adaptation response per story.
...
- **Adaptation:** Alef, Alef Plus, Bet, modern spoken Hebrew, target lengths, lexical units, RU/EN translation, no filler, and a separate DIALOG-only literal structure example loaded only for dialogue calls.
...
Generation has four content-producing prompt stages plus a binding sourced-review gate. Sourced discovery receives only CURRENT/HISTORY editorial, search, source, and sourced-novelty rules. After every successful sourced search response, a separate no-web duplicate-review request receives only compact candidate, recent sourced, and already selected sourced records. Selected-HISTORY research receives only the chosen subjects or unresolved reserve replacements and uses web search. Generated planning receives only EVERYDAY/DIALOG scenario and generated-novelty rules and never uses web search. Adaptation receives only writing, dialogue-rendering, segmentation, translation, and configured level/locale rules for the frozen briefs, scenario metadata, and private HISTORY beat contracts. Alef keeps the same length targets and factual/scenario development while using more frequent concrete vocabulary, active clauses, simpler forms, and more short sentences instead of deleting material. After each new adaptation, generation restores a neutral final full stop only where a teaser or body paragraph lacks terminal punctuation, then applies new-content-only checks: meaningful lexical units contain no more than four whitespace-delimited words, and a sentence-starting unit of three or more words cannot also end that sentence; separator units cannot contain alphabetic text, and DIALOG must contain 8–12 labeled direct-speech turns with stable alternating speakers. Archived issues remain valid without migration. It processes exactly one article per request so an article that remains invalid after its retry can be omitted without affecting valid siblings or later articles. API/request failure on the final attempt remains fatal, as does final combined-issue validation. If an append retains no valid new articles, it leaves the existing files untouched; if a fresh run retains none, it exits successfully without creating an empty issue. Generation keeps the configured `OPENAI_MODEL` for every stage and review.
...
For the normal eleven sourced slots, each discovery attempt returns exactly 36 compact candidates containing an ID, type, category, internal history-family classification, internal discovery lane, factual English brief, and source metadata. Discovery never returns HISTORY story beats. Candidate IDs should be descriptive topic slugs, but Python defensively adds the issue date as an idempotent suffix before validation and publication so response-local placeholders such as `current-01` cannot collide with another day's story. When both types remain, discovery returns exactly 12 CURRENT and 24 HISTORY candidates. The first two passes search across Israel rather than defaulting to Jerusalem and allocate HISTORY leads across the four documented Israeli discovery lanes; the third pass is a worldwide fallback for still-open sourced slots. CURRENT focuses on the target date and previous several days. HISTORY first considers worthwhile date-related facts, then prioritizes people, historic Israeli companies and manufacturing, culture, and concrete events without requiring an anniversary or current-news connection. The HISTORY pool contains at least six `person`, six `israeliIndustry`, six `culture`, and four `event` records; `place` is capped at two and `archaeology` at one. Python validates candidates individually in response order against recent records, already retained records, and earlier valid candidates from the same response. Candidate-specific URL, slug, brief, metadata, and duplicate failures remove only the affected candidate. Pool-count, discovery-lane, and history-family-mix failures are logged and merged into retry feedback but do not discard individually valid candidates. Deterministic family selection chooses the available final HISTORY mix before another discovery attempt fills remaining slots, and reviewed unselected HISTORY candidates become reserves. The internal family and discovery-lane fields are removed before adaptation and are never stored in issue JSON.
...
After selection, one batched web request researches all chosen HISTORY subjects. Each requested ID must appear exactly once. Valid partial results are retained; malformed, missing, duplicate, thin, or explicitly insufficient packs remain unresolved. Every sufficient pack contains 8–12 unique English beats with stable IDs, narrative roles, and required flags. Required beats collectively cover setup, action, turning point, and outcome. Sources and per-beat supporting URLs are optional; trustworthy consulted URLs are retained when available, and missing or discarded URLs never invalidate the factual beats. Each unresolved subject is replaced in place by the next compatible reviewed reserve when available; `event` and `place` are compatible for their shared final family slot, while every other replacement must match its family. Israel-focused reserves precede worldwide reserves because discovery appends each reviewed pass in order. Research repeats only for unresolved replacements until a slot succeeds or its compatible reserves are exhausted, and each reserve can be consumed once. Two consecutive research-request failures abort the run; a successful response resets that failure count. Generated planning creates no more than the normal four EVERYDAY/DIALOG stories for a fresh issue. Generated duplicates and repeated same-issue canonical domains are removed individually, and only the remaining generated slots are retried. Type counts are editorial targets, not publication-blocking quotas, and a final shortfall does not by itself fail publication when usable unique stories remain.

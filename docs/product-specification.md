...
A historical person, company, factory, institution, cultural work, event, or place is a subject, not yet a story. Discovery must gather enough sourced material to retell what actually happened: concrete actions, decisions, working methods, problems, changes, turning points, consequences, and outcomes as applicable. A shallow profile, roundup, exhibition page, or event listing is a lead; while the same web-enabled discovery call is active, follow that lead through additional specific source pages rather than retaining only its promotional summary. Do not replace the historical material with claims that the subject “shows,” “reflects,” or “represents” society, culture, memory, influence, or importance.
...
For a new complete issue, two EVERYDAY and two DIALOG stories are editorial targets, not validation requirements. The normal sourced target is four CURRENT and seven HISTORY stories. Discovery returns 12 CURRENT and 24 HISTORY screening candidates before filtering. Only after that pool has been searched and filtered may an unavailable, weak, or duplicate sourced slot be replaced by an EVERYDAY or DIALOG story. The research prompt also receives compact date/type/category/ID/brief summaries from all issues in the previous three calendar days so CURRENT events, HISTORY subjects, EVERYDAY plots, and DIALOG situations are not repeated on consecutive days. Recent generated-scenario records are explicitly forbidden comparison data; an identical scenario ID or a renamed version with the same practical problem, interaction, and resolution must be discarded.
...
CURRENT and archived HISTORY stories may store real publisher, title, and URL data when trustworthy canonical links are available. Missing, duplicate, or unverified links are discarded and an empty source list remains valid for archived content and CURRENT under the existing compatibility policy. A newly discovered HISTORY candidate with `storyBeats` must retain at least one verified source after cleanup, because unsupported beats cannot serve as its factual contract; sourcing must correct or replace it before adaptation. EVERYDAY and DIALOG have no sources. Never create fictional links or present generated scenarios as news. The content type must be explicit in stored data.
...
A daily JSON file contains its date, available reading-level IDs, available translation-locale codes, and ordered stories. Each story contains its type, category, an English internal brief used for duplicate comparison, a source list that may be empty, an optional sourced-image object, its configured level variants, teaser, title, paragraphs, lexical annotations, and scenario metadata for EVERYDAY and DIALOG. Newly discovered HISTORY stories also retain 6–10 ordered English `storyBeats` as their source-supported factual contract; archived stories created before this field remain valid without backfilling. If stored, beats are valid only on HISTORY and must be non-empty English strings. `index.json` lists available dates. `everyday-history.json` supports diversity checks for both generated types. Site configuration declares defaults and enabled locales; the reading-level configuration defines ordered adaptation bands; locale dictionaries contain interface copy.
...
Return 24 distinct HISTORY screening candidates from a substantially larger search and normally select seven. Classify each as `person`, `israeliIndustry`, `culture`, `event`, `place`, or `archaeology`; this internal field is used for selection and removed before publication. Require at least six person, six Israeli-industry, six culture, and four event candidates. Permit at most two place candidates and one archaeology candidate. The selected seven must contain at least two person, two Israeli-industry, two culture, and one event or exceptional place story, with at most one place and no archaeology. Favor concrete stories that are interesting, easy to explain, and rich in useful ordinary language. They need not relate to the current date or current events. During the existing sourcing call, give every HISTORY candidate 6–10 ordered, source-supported beats that collectively form a retellable narrative. When the first source is shallow, search additional specific pages about the same subject instead of reducing the result to a topic summary. This requirement adds research material to the existing response and does not introduce another API phase or call.

## 31. Facts and generation

Language adaptation must not freely invent details for real stories. A factual brief must precede all CURRENT versions; a compact brief and ordered factual beats must precede new HISTORY versions. HISTORY adaptation retells those beats rather than describing the source or offering generic commentary about the subject. If evidence is limited, make the article shorter. Never hallucinate details to reach a length target.

## 32. OpenAI API

Use the OpenAI API for sourced discovery and selection, sourced semantic duplicate review, generated-scenario planning, level adaptation, lexical segmentation, and Russian and English translations. Use the current official SDK and a modern API. A normal issue first makes web-enabled CURRENT/HISTORY discovery requests that each return exactly 36 screening candidates rather than final stories: CURRENT records stay compact, while HISTORY records also carry 6–10 factual story beats gathered during the same call. Every returned candidate batch then receives a separate no-web semantic duplicate review using only compact briefs and URLs before any candidate is retained. A separate no-web EVERYDAY/DIALOG planning request fills the remaining issue slots, followed by one structured adaptation response per story.
...
Keep editorial rules separate from application code. Maintain five instruction groups:
...
Generation has three content-producing prompt stages plus a binding sourced-review gate. Sourced discovery receives only CURRENT/HISTORY editorial, search, source, and sourced-novelty rules. After every successful sourced search response, a separate no-web duplicate-review request receives only compact candidate, recent sourced, and already selected sourced records. Generated planning receives only EVERYDAY/DIALOG scenario and generated-novelty rules and never uses web search. Adaptation receives only writing, dialogue-rendering, segmentation, translation, and configured level/locale rules for the frozen briefs, scenario metadata, and HISTORY story beats. Generation keeps the configured `OPENAI_MODEL` for every stage and review.

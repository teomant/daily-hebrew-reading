...
This normally produces 12 stories and may range from 10–13 as the number of sourced stories varies. Treat two EVERYDAY and two DIALOG entries as editorial targets; never fill the flexible CURRENT or HISTORY allowance with weak material.
...
A real historical story, normally four per issue. The normal sourced pool contains 16 HISTORY candidates gathered from substantially more source pages before filtering, then selects the four most interesting unique candidates. It must include at least four real historical events, four notable-person stories, and four stories about nature sites, national parks, gardens, trails, viewpoints, museums, landmarks, unusual local attractions, or tourist destinations in Israel; the remaining candidates carry the strongest varied subjects. Artists, writers, scientists, educators, engineers, athletes, founders, guides, and community figures are useful person-led subjects. Archaeological excavations, ancient streets, building layers, pottery finds, and construction-site discoveries are a limited fallback rather than the default: no more than two may appear in the 16-candidate pool, and no more than one may appear among its first four HISTORY candidates. Broader relatable world history remains acceptable on a retry.
...
Return 16 distinct HISTORY screening candidates from a substantially larger search and normally select the best four. Include at least four real past events, four notable-person stories, and four Israeli nature/park/landmark/destination stories. The first four HISTORY candidates must span all three preferred families. At most two of the 16 candidates, and at most one of the first four, may use an archaeological excavation or materially equivalent discovery as the main hook. Favor concrete stories that are interesting, easy to explain, and rich in useful ordinary language. They need not relate to the current date or current events. Gather facts and sources before writing the adaptations.
...
Use the OpenAI API for sourced discovery and selection, sourced semantic duplicate review, generated-scenario planning, level adaptation, lexical segmentation, and Russian and English translations. Use the current official SDK and a modern API. A normal issue first makes web-enabled CURRENT/HISTORY discovery requests that each return exactly 28 compact screening candidates rather than final stories. Every returned candidate batch then receives a separate no-web semantic duplicate review before any candidate is retained. A separate no-web EVERYDAY/DIALOG planning request fills the remaining issue slots, followed by one structured adaptation response per small story batch.
...
For the normal eight sourced slots, each discovery attempt returns exactly 28 compact candidates containing only an ID, type, category, factual English brief, and source metadata. When both types remain, it returns exactly 12 CURRENT and 16 HISTORY candidates and searches across the whole of Israel rather than defaulting to Jerusalem. CURRENT focuses on the target date and previous several days. HISTORY first considers worthwhile date-related facts, then deliberately diversifies across events, people, nature sites, parks, landmarks, interesting Israeli places, and destinations without requiring an anniversary or current-news connection. Archaeological discovery stories are capped at two of the 16 HISTORY candidates and one of their first four. Python first applies URL, slug, and brief checks. The separate LLM reviewer must then classify every remaining candidate exactly once from its brief and URLs, comparing underlying events and named historical subjects rather than wording. Its duplicate verdicts are binding and removed candidates are never adapted or published. Missing, malformed, or failed review coverage fails closed by discarding the unreviewed batch, but does not fail publication: discovery retries, and generated stories fill any final shortfall. A retry must start a genuinely new worldwide replacement search; at least 21 of its 28 candidates should be outside Israel and span at least six countries or regions. It must not return alternate coverage, translations, updates, or renamed versions of rejected stories. All editorial, safety, source, and novelty rules remain active. After sourced discovery ends, generated planning fills the issue to its target with EVERYDAY and DIALOG stories. Generated duplicates are removed and only the remaining slots are retried. Type counts are editorial targets, not publication-blocking quotas, and a final shortfall does not by itself fail publication when usable unique stories remain.

...
Generation must send the research LLM explicit forbidden-story records for the existing issue and recent issues, including IDs, English briefs, and normalized source URLs. The prompt treats those records as prohibited output rather than examples: the LLM must not copy, rewrite, translate, update, continue, rename, or add a date/year suffix to them. It must compare candidates semantically and reject the same subject even when the language, publisher, URL, headline, wording, angle, small update, or detail differs; uncertainty is resolved by choosing an unrelated generated scenario. Python independently compares every new candidate against both the same-day issue and the recent-issue records, rejecting duplicate or near-duplicate slugs, briefs, and source URLs before adaptation. When duplicate candidates are found, generation retains the unique candidates and requests AI-generated EVERYDAY or DIALOG replacements only for the rejected slots, without web search. If the final replacement still contains duplicates but the remaining unique new-issue batch is within the configured total range, generation proceeds with that smaller valid batch instead of failing. Generation runs are serialized so simultaneous scheduled or manual invocations cannot race and overwrite one another. A failed append leaves the existing issue unchanged.

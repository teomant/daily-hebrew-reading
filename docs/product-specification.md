...
For a new complete issue, three EVERYDAY and three DIALOG stories are editorial targets, not validation requirements. The model should also try to find the normal CURRENT and HISTORY mix, but a sourced candidate that is unavailable, weak, or rejected as a duplicate is replaced by an EVERYDAY or DIALOG story. The research prompt also receives compact date/type/category/ID/brief summaries from all issues in the previous three calendar days so CURRENT events, HISTORY subjects, EVERYDAY plots, and DIALOG situations are not repeated on consecutive days.

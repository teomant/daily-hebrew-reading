...
Use the OpenAI API for sourced discovery and selection, generated-scenario planning, level adaptation, lexical segmentation, and Russian and English translations. Use the current official SDK and a modern API. A normal issue first makes web-enabled CURRENT/HISTORY discovery requests, then a separate no-web EVERYDAY/DIALOG planning request for the remaining issue slots, followed by one structured adaptation response per small story batch.
...
Generation has three isolated prompt stages. Sourced discovery receives only CURRENT/HISTORY editorial, search, source, and sourced-novelty rules. Generated planning receives only EVERYDAY/DIALOG scenario and generated-novelty rules and never uses web search. Adaptation receives only writing, dialogue-rendering, segmentation, translation, and configured level/locale rules for the frozen briefs. Generation keeps the configured `OPENAI_MODEL` for every stage.

Each planning stage receives only its relevant compact forbidden records from the existing issue and recent issues. Sourced discovery defines a duplicate as the same central subject and underlying event, action, announcement, change, project, or outcome; equivalent URLs, alternate coverage, cosmetic rewrites, and minor follow-ups remain duplicates. HISTORY duplicates tell the same specific historical story, while merely sharing a place is allowed when the subject is genuinely different. Generated planning defines a duplicate by the practical problem or goal, interaction, and resolution; an identical recent scenario ID is always a duplicate, while sharing only a domain or vocabulary is allowed. Forbidden records are comparison data, never search suggestions or templates.

For the normal six sourced slots, discovery searches surplus CURRENT and HISTORY candidate pools, covering the whole of Israel rather than defaulting to Jerusalem. CURRENT focuses on the target date and previous several days. HISTORY first considers worthwhile date-related facts, then unrelated short and interesting Israeli history without requiring an anniversary or current-news connection. Python compares every returned candidate with recent same-type records and already selected candidates. Unique sourced candidates are retained; duplicates cause a second fresh web-search request for the missing sourced types. After sourced discovery ends, generated planning fills the issue to its target with EVERYDAY and DIALOG stories. Generated duplicates are removed and only the remaining slots are retried. Type counts are editorial targets, not publication-blocking quotas, and a final shortfall does not by itself fail publication when usable unique stories remain.

Retries are fresh Responses API requests containing compact retained records and exact validation feedback. They do not use `previous_response_id`, because inherited input remains billable and stale rejected output can anchor the correction. Before every OpenAI request, generation logs the complete phase-specific system and user prompt with role and phase markers. Python also rejects duplicate or near-duplicate slugs, briefs, source URLs, and exact generated scenario IDs before adaptation. Generation runs are serialized so simultaneous scheduled or manual invocations cannot race and overwrite one another. A failed append leaves the existing issue unchanged.

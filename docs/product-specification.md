...
After selection, one batched web request researches all chosen HISTORY subjects. Each requested ID must appear exactly once. Valid partial results are retained; malformed, missing, duplicate, thin, or explicitly insufficient packs remain unresolved. Every sufficient pack contains 8–12 unique English beats with stable IDs, narrative roles, and required flags. Required beats collectively cover setup, action, turning point, and outcome. Sources and per-beat supporting URLs are optional; trustworthy consulted URLs are retained when available, and missing or discarded URLs never invalidate the factual beats. On the first attempt, each unresolved selected subject is replaced in place by one compatible reviewed reserve when available. Only unresolved replacements enter the second and final batched research request, and each reserve can be consumed once. After research, generated planning fills any issue shortfall with EVERYDAY and DIALOG stories. Generated duplicates are removed and only the remaining slots are retried. Type counts are editorial targets, not publication-blocking quotas, and a final shortfall does not by itself fail publication when usable unique stories remain.

...
Discovery and adaptation use separate system prompts. Discovery receives editorial and scenario-planning rules but no Hebrew adaptation or lexical-segmentation instructions; adaptation receives only writing, dialogue-rendering, segmentation, and translation rules for frozen briefs. Generation keeps the configured `OPENAI_MODEL` for both phases.

Generation must send the research LLM explicit forbidden-story records for the existing issue and recent issues, including IDs, English briefs, and normalized source URLs. A single novelty contract defines a sourced duplicate as the same central subject and underlying event, action, announcement, change, project, or outcome. It defines a generated-scenario duplicate by the practical problem or goal, interaction, and resolution; an identical recent scenario ID is always a duplicate. The same historical place, everyday domain, or vocabulary alone is not a duplicate when the actual subject or scenario differs. Equivalent source URLs and alternate reports of the same underlying event are duplicates. Cosmetic changes such as another headline, publisher, language, slug, names, date, angle, or minor follow-up do not make a duplicate new. The LLM must use forbidden records only for comparison, never as search suggestions or templates. For the normal six sourced slots, discovery must inspect separate surplus pools containing at least eight CURRENT candidate pages and at least four Israeli HISTORY candidate pages across unrelated permitted areas. Every matching candidate is discarded and does not count toward its pool; discovery continues searching for another candidate of that type and returns only the final unique stories. HISTORY candidates need no connection to the publication date or current events. The model may fill missing sourced slots with unrelated generated scenarios only after inspecting both surplus pools. For sourced stories, completion also requires a clear central subject and event, a supported brief, and a passed novelty comparison. Python independently compares every new candidate against both the same-day issue and the recent-issue records, rejecting duplicate or near-duplicate slugs, briefs, and source URLs before adaptation. Before every OpenAI request, generation writes the complete phase-specific system and user prompt to the workflow log with role and phase markers so production behavior can be debugged. When duplicate candidates are found, generation retains the unique candidates and requests AI-generated EVERYDAY or DIALOG replacements only for the rejected slots, without web search. If the final replacement still contains duplicates but the remaining unique new-issue batch is within the configured total range, generation proceeds with that smaller valid batch instead of failing. Generation runs are serialized so simultaneous scheduled or manual invocations cannot race and overwrite one another. A failed append leaves the existing issue unchanged.

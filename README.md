...
Discovery receives only the editorial screening prompt and does not create HISTORY story beats. After duplicate review and selection, the selected-HISTORY research prompt gathers at least two actually consulted source pages and freezes 8–12 ordered, source-supported beats with required setup, action, turning-point, and outcome roles. Adaptation must retell those beats instead of substituting a résumé, source summary, institutional promotion, or generic statement about importance or legacy. Every newly researched HISTORY level must report coverage of every required beat and reach its configured `minimumWords`; either failure retries and then blocks publication. Simpler levels may compress secondary detail but cannot drop the narrative spine. Non-HISTORY word ranges remain editorial guidance rather than a publication gate. The finished Hebrew uses mostly one-to-three-word translation chunks with longer units reserved for indivisible expressions and names. Ordinary spaces are restored locally instead of being represented by verbose JSON separator objects. Individual empty translations are allowed, but each story level must reach at least 75% coverage per language.

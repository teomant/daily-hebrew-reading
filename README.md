...
Generation logs timestamp each research, adaptation, validation, and write phase, including elapsed API-call time and concise retry errors. If research is valid but an adaptation fails validation, only adaptation is retried; the verified factual briefs are not researched again.

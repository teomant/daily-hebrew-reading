...
- Three-stage OpenAI Responses API generation: web-enabled CURRENT/HISTORY discovery freezes compact duplicate-identification briefs plus 6–10 concrete story beats for every HISTORY candidate, separate EVERYDAY/DIALOG planning fills the remaining slots, then one adaptation request per story retells the frozen material in Hebrew before adding lexical units and contextual translations.
...
- A 36-candidate sourced discovery pool before filtering: 12 compact CURRENT and 24 research-backed HISTORY candidates. HISTORY is deliberately balanced toward historical people, Israeli companies and manufacturers, and culture, with concrete events in support; generic park, destination, and archaeology stories are tightly capped. The existing discovery call follows shallow HISTORY leads through additional specific pages and records ordered actions, decisions, changes, turning points, and outcomes rather than adding another research phase. Invalid or duplicate candidates are removed individually, while an imperfect pool mix becomes retry guidance instead of discarding the whole batch. Duplicate sourced candidates trigger another web-search attempt before generated stories fill any remaining slots.
...
Discovery receives only the editorial, everyday-scenario, and dialogue-planning prompts; adaptation receives only the Hebrew writing, dialogue rendering, segmentation, and translation prompt. Each HISTORY discovery record separates a compact deduplication brief from 6–10 ordered, source-supported story beats. Adaptation must retell those beats instead of substituting a résumé, source summary, institutional promotion, or generic statement about importance or legacy. Every level preserves the principal actions, consequential change or turning point, and outcome while simpler levels may compress secondary detail. Before segmentation, adaptation estimates the Hebrew body length and makes one supported revision toward the configured target when it is short. It must return a coherent shorter version when the frozen material cannot support more detail without invention, repetition, or filler; being below the suggested word range alone never fails generation. The finished Hebrew then uses mostly one-to-three-word translation chunks with longer units reserved for indivisible expressions and names. Ordinary spaces are restored locally instead of being represented by verbose JSON separator objects. Individual empty translations are allowed, but each story level must reach at least 75% coverage per language.

...
- Four-stage OpenAI Responses API generation: web-enabled CURRENT/HISTORY discovery returns compact screening records, strict duplicate review and deterministic selection choose the publishable subjects, batched web-enabled requests deeply research only selected HISTORY stories and compatible reserves into 8–12 supported narrative beats, separate EVERYDAY/DIALOG planning creates the normal generated allocation, and adaptation retells the frozen material in Hebrew with lexical units and contextual translations.
- A normal 4 CURRENT / 7 HISTORY / 2 EVERYDAY / 2 DIALOG target mix, with each dialogue turn on its own line. Missing sourced stories reduce the issue size instead of creating more than the normal four generated stories.
- A 36-candidate sourced discovery pool before filtering: 12 compact CURRENT and 24 compact HISTORY candidates. The first two attempts focus on Israeli material and divide HISTORY discovery across Wikimedia references, the National Library/Historical Jewish Press, state visual archives, and Israeli culture archives; a third attempt may search worldwide only for sourced slots that remain open. HISTORY is deliberately balanced toward historical people, Israeli companies and manufacturers, and culture, with concrete events in support. Invalid or duplicate candidates are removed individually, while an imperfect pool mix becomes retry guidance instead of discarding the whole batch. Reviewed but unselected HISTORY candidates are retained as reserves, with Israeli candidates ahead of worldwide fallbacks. Deep research keeps consuming compatible reserves until the slot succeeds or that reserve family is exhausted.
...
- `prompts/*.md` — sourced editorial, selected-HISTORY research, deduplication, everyday, dialogue, and adaptation rules.
...
Generation logs timestamp each research, adaptation, validation, and write phase. Immediately before each OpenAI call, it logs the complete phase-specific system and user prompts. Sourced discovery has up to three attempts: two Israel-focused passes and one final worldwide fallback. Selected HISTORY research is one batched web call in a normal run and continues in batches for unresolved compatible reserve replacements; two consecutive request failures abort the run. Generated scenarios use distinct canonical domains within the current issue and prompts vary interaction shapes beyond repeated delay, delivery, missing-item, repair, and office plots. Adaptation runs one story per request. DIALOG calls alone receive a literal eight-turn `speaker: words` example, and new dialogue output must contain 8–12 labeled alternating direct-speech turns at every level. Invalid article output retries once, then that article is logged and omitted while later articles continue; an API/request failure on the final attempt remains fatal.
...
Discovery receives only the editorial screening prompt and does not create HISTORY story beats. After duplicate review and selection, the selected-HISTORY research prompt freezes 8–12 ordered factual beats with required setup, action, turning-point, and outcome roles. Source links are retained when trustworthy URLs are available, but missing or discarded links do not invalidate an otherwise usable HISTORY pack. Adaptation must retell the beats instead of substituting a résumé, source summary, institutional promotion, or generic statement about importance or legacy. Every newly researched HISTORY level must report coverage of every required beat; missing coverage retries once and then omits only that article. Word ranges remain editorial guidance for every story type, so a coherent short article never fails generation. Alef keeps its existing target length but uses simpler, frequent vocabulary and more short active sentences rather than deleting material. New adaptations receive missing terminal full stops automatically; alphabetic separator units that could glue Hebrew words together are rejected and retried. These quality rules do not retroactively invalidate archived issues. The finished Hebrew uses mostly one-to-three-word translation chunks with longer units reserved for indivisible expressions and names. Ordinary spaces are restored locally instead of being represented by verbose JSON separator objects. Individual empty translations are allowed, but each story level must reach at least 75% coverage per language.
